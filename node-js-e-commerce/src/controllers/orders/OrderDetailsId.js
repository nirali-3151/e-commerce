...
function getOrderID(req, res, next) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const sql = `SELECT * FROM orders WHERE o_user_id = ${decoded.user_id} AND order_id= ${orderId}`
...
//get address data
function get_address_details(req, res, next, order_id) {
    return new Promise((resolve, reject) => {
        const sql = `SELECT * from address where address_id = ${order_id[0].o_address_id}`
            conn.query(sql, async function (err, rows, feild) {
                if (err) throw err
                resolve(rows.rows)
            })
...
async function makeSynchronousRequest(req, res, next) {
    try {
        ...
        //get address details
        let http_promise6 = get_address_details(req, res, next, order_id);
        let address_details = await http_promise6;
        ...
        size_details = await size_details.reduce((acc, current) => {
        ...
        colors_details = await colors_details.reduce((acc, current) => {
...
exports.userOrderDetailsWithID = async (req, res, next) => {
